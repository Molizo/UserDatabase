...
struct settings
{
    ...
    string serverIP;int enableNetworking,getUsersAtStartup,sendUsersAtExit;
    int firstRun;
};
settings setting;
...
{string input,input2,input3;
    ...
    if(input2 == input3 && input == setting.username)
    {
        ...
        system("pause");
        system("cls");
        out<<setting.firstRun<<" "<<setting.foregroundColor<<" "<<setting.backroundColor<<" "<<setting.autosave<<" "<<setting.importOnStartup<<" "<<setting.username<<" "<<setting.userPassword<<" "<<setting.autoLogin<<" "<<setting.enableNetworking<<" "<<setting.getUsersAtStartup<<" "<<setting.sendUsersAtExit<<" "<<setting.serverIP;
    }
    else
    {
...
int settingsimport()
{
    ...
    in>>setting.firstRun>>setting.foregroundColor>>setting.backroundColor>>setting.autosave>>setting.importOnStartup>>setting.username>>setting.userPassword>>setting.autoLogin>>setting.enableNetworking>>setting.getUsersAtStartup>>setting.sendUsersAtExit>>setting.serverIP;
}
...
int sendUsersAtExitConf()
{
    char input;
    cout<<"\n You must have Networking enabled to use this feature\n Enable sending Users to Server on Exit? (Yes/No) ";cin>>input;
    if(input == 'y' || input == 'Y')
        setting.sendUsersAtExit = 1;
    else
        setting.sendUsersAtExit = 0;
    system("cls");
}

int getUsersAtStartupConf()
{
    char input;
    cout<<"\n You must have Networking enabled to use this feature\n Enable getting Users from Server on Startup? (Yes/No) ";cin>>input;
    if(input == 'y' || input == 'Y')
        setting.getUsersAtStartup = 1;
    else
        setting.getUsersAtStartup = 0;
    system("cls");
}
...
{int input;
    ...
    while(true)
    {
        ...
        cout<<"  4.Change Autosave setting\n";
        cout<<"  5.Enable Automatic Login on Startup\n";
        cout<<"  6.Enable Networking\n";
        cout<<"  7.Enable Send Users to Server on Exit\n";
        cout<<"  8.Enable Get Users from Server on Startup\n";
        cout<<"  9.Change Server IP\n";
        cout<<"  10.Change Restricted Account Username\n";
        cout<<"  11.Change Restricted Account Password\n";
        cout<<"  12.Go back\n";
        ...
        if(input==7)    sendUsersAtExitConf();
        if(input==8)    getUsersAtStartupConf();
        if(input==9)
        {
        ...
        if(input==10)    changeUser();
        if(input==11)   changePassw();
        if(input==12)   break;
    }
    out<<setting.firstRun<<" "<<setting.foregroundColor<<" "<<setting.backroundColor<<" "<<setting.autosave<<" "<<setting.importOnStartup<<" "<<setting.username<<" "<<setting.userPassword<<" "<<setting.autoLogin<<" "<<setting.enableNetworking<<" "<<setting.getUsersAtStartup<<" "<<setting.sendUsersAtExit<<" "<<setting.serverIP;
}
...
{
    system("cls");
    cout<<"Microsoft Windows User Database Setup\n===================================== \n\n";
...
int login()
{
    ...
    while(true)
    {
        ...
        {
            cout<<"404 Error: User not found on domain.\n\n";
            Sleep(250);
            cout<<"Please try again.\n\n\n";
            system("pause");
        }
...
{
    system("cls");
    cout<<"Microsoft Windows User Database\n=============================== \n\n";
...
{char input;
    ...
    if(input=='y'|| input=='Y')
    {
        ...
        fout<<"Microsoft Windows User Database\n=============================== \n\n";
...
int addUser()
{
    if(setting.enableNetworking == 1 && setting.getUsersAtStartup == 1)
    {
        ftpScript(31);
    ...
    if(setting.enableNetworking == 1 && setting.sendUsersAtExit == 1)
    {
        exportToFile(2);
        ftpScript(32);
        system("ftp /s:FTP.txt /A");
...
int user_Restricted()
{
    ...
    if(menuOption == 5) changeUser();
    if(menuOption == 6) changePassw();
    if(menuOption == 7)
    {
        loading_screen_logout();
        login();
...
int startupRoutine()
{
    ...
    if(setting.importOnStartup == 1 && (setting.enableNetworking == 0 || setting.getUsersAtStartup == 0))    importFromFile(0);
    if(setting.enableNetworking == 1 && setting.getUsersAtStartup == 1)
    {
        ftpScript(31);
        system("ftp /s:FTP.txt /A");
    ...
    system("title Microsoft Windows User Database Client");
